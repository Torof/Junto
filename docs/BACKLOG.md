...
### Niveau auto-déclaré validé par les pairs (conçu 2026-07-08, ✅ livré — migs 00293–00299)
...
**Découpage :**
- **Étape 1 — cohérence — ✅ LIVRÉ 2026-07-08** : garde-fou `give_reputation_badge` (mig 00293 : pas de vote `level_*` sans niveau déclaré pour le sport de l'activité, refus générique) + `get_activity_peer_review_state` expose `declared_level` par participant (mig 00294, depuis `public_profiles`) + client : peer-review masque la section niveau quand `declared_level` null + tooltip profil n'affiche le verdict que si niveau déclaré. Trou de sécurité fermé (smoke test anon OK). `level_under` était déjà retiré côté vote (client + serveur). Reste cosmétique non urgent : l'agrégation `under` (toujours 0 désormais) et `fiable = right + under` dans badge-display → passer à `fiable = right`.
- **Étape 2 — le système — ✅ LIVRÉ 2026-07-08** : `set_sport_level` (mig 00295, SECURITY DEFINER — première déclaration libre, un cran à la fois, montée si net `right − over` ≥ 3 sur 12 mois, descente libre, reset des votes au changement, aucune suppression, self-only, advisory-lock) + `levels_per_sport` **privilégiée** (verrou whitelist dans `handle_user_update`, plus aucune écriture directe) + client : drawer **« Gérer »** (ajout via la fonction, sports déclarés en lecture seule, mise en garde d'honnêteté à la 1ʳᵉ déclaration, « ? » explicatif) + boutons owner **Monter/Redescendre** dans le tooltip (profil propre uniquement). **Pas** de table `sport_level_scale` — le niveau utilisateur est le 4-tiers fixe (`débutant<intermédiaire<avancé<expert`). Erreur `junto.level_up_locked` (i18n FR+EN). Smoke tests anon OK.
