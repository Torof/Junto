-- ============================================================================
-- 00395 — Expand the dispo vibe vocabulary (Scott 2026-08-29): richer signals
-- of what a person is looking for. 12 → 25 closed values (still ≤6 selectable).
-- Adds: nature, challenge, photo (ambiance); mixed, same_level, beginners
-- (company); long_outing, after_work, regular (pace); adapted, training,
-- experienced, competition (profile/access). Gear pills are parked for later.
--   • widen discovery_availabilities_intent_check
--   • widen upsert_dispo's intent validation (reproduced from 00394)
--   • refresh demo dispos to showcase new pills
-- ============================================================================

-- ---------- Widen the closed vocabulary on the column ----------
...
-- ---------- upsert_dispo — widen intent validation (reproduced from 00394) ----------
...
-- ---------- Demo dispos: showcase a mix of new + old vibes ----------
UPDATE discovery_availabilities SET intent = ARRAY['performance','training','experienced','dog','active']
  WHERE user_id = 'd0000000-0000-4000-a000-000000000002';
UPDATE discovery_availabilities SET intent = ARRAY['detente','same_level','regular','solo','calm']
  WHERE user_id = 'd0000000-0000-4000-a000-000000000003';
UPDATE discovery_availabilities SET intent = ARRAY['discovery','nature','beginners','group','adapted']
