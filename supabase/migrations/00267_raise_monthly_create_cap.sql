-- Migration 00267: raise the monthly create_activity cap 15 → 30.
--
-- 00262 set 15/30d. That was needlessly tight: legitimate use of
-- create_activity is "I need partners for a specific outing" — rarely
-- >10/month — and a pro/club's regular program lives on the recurring
-- OFFERING catalog, a separate mechanism, not here. So the monthly cap
-- is pure spam-ceiling headroom; 15 only ever bit the dev dogfooding
-- (hit it at 18). 30 = 3× the legitimate ceiling, while the 10/24h
-- daily cap remains the real burst-spam guard. Body otherwise identical
-- to 00262.
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    ...
    IF v_monthly_count >= 30 THEN RAISE EXCEPTION 'Operation not permitted'; END IF;
  END IF;
