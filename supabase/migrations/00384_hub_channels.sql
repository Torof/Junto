-- ============================================================================
-- 00384 — Hub: surface channels (type='channel') alongside DM/group/activity.
-- Reuses the name + member_count columns (like groups); the client routes on
-- type='channel'. Closed channels still show (members keep read-only history).
-- ============================================================================
...
-- ---------- get_channel — metadata for the channel view / pre-join preview ----------
-- Metadata is public (needed to preview before joining); MESSAGES stay gated by
-- membership (get_conversation_messages).
...
BEGIN
  ...
  SELECT c.conversation_id, conv.name, c.sport_key,
         ST_X(c.base::geometry), ST_Y(c.base::geometry), c.base_label,
         c.description,
