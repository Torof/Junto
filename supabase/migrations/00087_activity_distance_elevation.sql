-- Migration 00087: add optional distance + elevation gain to activities
-- For outdoor sports (trail, hiking, cycling, ski-touring etc.) these are the
-- primary difficulty metrics, replacing or complementing the generic level.

ALTER TABLE activities
  ADD COLUMN distance_km NUMERIC(6, 1) CHECK (distance_km IS NULL OR (distance_km > 0 AND distance_km <= 9999)),
  ADD COLUMN elevation_gain_m INTEGER CHECK (elevation_gain_m IS NULL OR (elevation_gain_m > 0 AND elevation_gain_m <= 99999));

-- Expose in the activity views used by the client
...
-- Update create_activity to accept the new fields
DROP FUNCTION IF EXISTS create_activity(UUID, TEXT, TEXT, TEXT, INTEGER, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, TIMESTAMPTZ, TEXT, TEXT, BOOLEAN, FLOAT, FLOAT, TEXT);
