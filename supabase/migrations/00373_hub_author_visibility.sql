-- ============================================================================
-- 00373 — Audit M1: hub preview + is_unread must honour message_author_visible.
--
-- 00368's last-message LATERAL and the is_unread EXISTS filtered only on
-- deleted_at, so an activity conversation's hub row could show the last message
-- (text + sender) of a blocked/suspended author that the wall itself hides
-- (private.message_author_visible), plus a phantom unread badge. Add the author-
-- visibility filter to both. DM (whole conv already excluded when blocked) and
-- group (authors always visible) are unaffected — the helper is a no-op there.
-- Shape unchanged → CREATE OR REPLACE.
-- ============================================================================
