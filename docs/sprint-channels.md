...
> ⚠️ **CE DOC EST PÉRIMÉ (SUPERSEDED par mig 00400-00406, 2026-09-09).** Il décrit le modèle initial 00381-00385 (« 1 sport × zone plafonnée », référence `app/(auth)/channels.tsx`). **Modèle actuel** : un canal = **titre + zone** (centre + rayon **35/60/100 km**) + **0-1 sport optionnel** + **photo optionnelle** ; dédup **par zone** (sport-agnostique) ; **partage verrouillé au sport** du canal (`junto.channel_sport_mismatch`). La liste vit dans `src/components/channels-view.tsx` (hub Partenaires), pas dans `channels.tsx`. Les vibes/`intent` ont été **retirées** (00402). Chaînes d'autorisation à jour + bucket `channel-photos` + décision rideau démo : voir **`docs/SECURITY.md` → « Canaux (channels) »**. Historique du modèle : 00387 (1-sport×zone) → 00400 (multi-sport+vibes) → 00402 (drop vibes) → 00403 (photo) → 00404 (sport unique + verrou partage) → 00405/00406 (durcissements audit).
