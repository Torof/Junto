-- Migration 00096: GPX trace support for activities
-- Stores the uploaded trace as GeoJSON LineString in a jsonb column.
-- Kept as jsonb (not PostGIS geography) because we only need to render
-- the line on the map, no spatial queries on traces.

ALTER TABLE activities
  ADD COLUMN trace_geojson JSONB
  CHECK (
    trace_geojson IS NULL
    OR (
      trace_geojson->>'type' = 'LineString'
      AND jsonb_typeof(trace_geojson->'coordinates') = 'array'
      AND jsonb_array_length(trace_geojson->'coordinates') BETWEEN 2 AND 10000
    )
  );

-- Refresh all 3 views to expose trace_geojson
...
-- ============================================================================
-- Update create_activity to accept an optional trace_geojson jsonb parameter
-- New param is appended at the end with DEFAULT NULL, so old clients that
-- don't pass it continue to work unchanged.
-- ============================================================================
DROP FUNCTION IF EXISTS create_activity(
  UUID, TEXT, TEXT, TEXT, INTEGER, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT,
  TIMESTAMPTZ, TEXT, TEXT, BOOLEAN, FLOAT, FLOAT, TEXT, NUMERIC, INTEGER, TEXT
);
...
-- ============================================================================
-- RPC to update/clear the trace on an existing activity (creator only).
-- Passing NULL removes the trace.
-- ============================================================================
