-- ============================================================================
-- 00413 — get_my_conversations returns the channel photo_url so the messagerie
-- list can render a channel row like its channel card (photo when set, else the
-- sport-tinted square). (Scott 2026-09-09.)
-- ============================================================================

DROP FUNCTION IF EXISTS get_my_conversations();
CREATE FUNCTION get_my_conversations()
RETURNS TABLE (
  ...
  activity_id UUID, activity_title TEXT, sport_id UUID, photo_url TEXT
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    ...
    CASE WHEN c.type = 'channel' THEN ch.photo_url END
