...
export const activityService = {
  getNearby: async (bounds?: { swLng: number; swLat: number; neLng: number; neLat: number }): Promise<NearbyActivity[]> => {
    let query = supabase
      ...
      .select(
        'id, title, description, level, level_max, max_participants, starts_at, duration, status, visibility, sport_id, creator_id, lng, lat, start_lng, start_lat, meeting_lng, meeting_lat, end_lng, end_lat, objective_lng, objective_lat, objective_name, start_name, distance_km, elevation_gain_m, creator_name, creator_avatar, sport_key, sport_icon, sport_category, participant_count, requires_presence, trace_geojson',
      )
  ...
  create: async (form: ActivityFormData): Promise<string> => {
    ...
    const { data, error } = await supabase.rpc('create_activity', {
      ...
      p_level: form.level,
      p_level_max: form.level_max ?? undefined,
  ...
  update: async (activityId: string, fields: {
    ...
    const { error } = await supabase.rpc('update_activity' as 'join_activity', {
      ...
      p_level_max: fields.level_max ?? null,
  ...
  updateTrace: async (activityId: string, trace: GeoJsonLineString | null): Promise<void> => {
    const { error } = await supabase.rpc('update_activity_trace' as 'join_activity', {
      p_activity_id: activityId,
      p_trace_geojson: trace ?? null,
    } as unknown as { p_activity_id: string });
    if (error) throw error;
  },
};
