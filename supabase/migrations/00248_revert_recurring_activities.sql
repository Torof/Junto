-- Migration 00248: revert the "recurring activity" abstraction.
--
-- After product discovery (outdoor pros operate on demand, indoor pros
-- run scheduled classes — both belong in a catalog, not a scheduled
-- event), the recurring-activity concept introduced in 00240/00245/
-- 00246/00247 is being replaced with a `pro_offerings` catalog entity
-- (migrations 00249+). Nothing references is_recurring in production
-- usage yet, so the rollback is clean.
--
-- Reverts:
--   - Drops reset_completed_recurring_activities + its hourly cron job
--   - Drops the is_recurring + recurrence_days columns from activities
--     (plus the CHECK constraint that ties them together)
--   - Restores peer_validate_presence + give_reputation_badge to their
--     pre-00245 bodies (no is_recurring branch)
--   - Restores create_activity to its pre-00246 signature (no recurring
--     params), identical to 00144's body
--   - Restores the four activity views to their pre-00247 shape (no
--     is_recurring / recurrence_days columns in the SELECT lists)

-- ============================================================================
-- 1. Unschedule + drop the hourly reset job
-- ============================================================================
DO $$
BEGIN
...
DROP FUNCTION IF EXISTS reset_completed_recurring_activities();

-- ============================================================================
-- 2. Restore peer_validate_presence (00237 body, no is_recurring branch)
-- ============================================================================
...
-- ============================================================================
-- 3. Restore give_reputation_badge (00237 body, no is_recurring branch)
-- ============================================================================
...
-- ============================================================================
-- 4. Restore create_activity to pre-00246 signature (00144 body)
--    Must DROP first because the parameter list changes (Postgres treats
--    different param signatures as overloads).
-- ============================================================================
DROP FUNCTION IF EXISTS create_activity(
  UUID, TEXT, TEXT, TEXT, INTEGER, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT, FLOAT,
  TIMESTAMPTZ, TEXT, TEXT, BOOLEAN, FLOAT, FLOAT, TEXT, NUMERIC, INTEGER, TEXT, JSONB,
  BOOLEAN, INTEGER
);
...
-- ============================================================================
-- 5. Restore the four activity views to their pre-00247 shape
--    (no is_recurring / recurrence_days columns)
-- ============================================================================
...
-- ============================================================================
-- 6. Drop the is_recurring / recurrence_days columns from activities.
--    Done last so the function + view restores above don't reference dead
--    columns mid-migration. Drop the cross-column CHECK explicitly first;
--    the per-column CHECK on recurrence_days drops with the column.
-- ============================================================================
ALTER TABLE activities
  DROP CONSTRAINT IF EXISTS activities_recurring_has_days;
