-- Migration 00233: lock transport, gear, and seat-request logistics
-- once the activity has started.
--
-- Symptom (Scott, 2026-05-14): set_participation_transport and
-- set_activity_gear (and the four seat-request RPCs) all accept
-- `status IN ('published', 'in_progress')`. That allows edits during
-- the in_progress window, which is wrong: once the activity starts,
-- the logistical decisions are sealed — switching driver / dropping a
-- shared gear item mid-event creates inconsistent state and is open
-- to abuse (no-shows revising their transport claim retroactively to
-- duck a reliability ding, etc.).
--
-- Fix: add a time guard `AND starts_at > NOW()` next to the existing
-- status check on every logistics-mutating RPC. Bodies otherwise
-- identical to their most recent published versions:
--   - set_participation_transport — 00200
--   - set_activity_gear          — 00197
--   - request_seat               — 00215
--   - accept_seat_request        — 00217
--   - decline_seat_request       — 00200
--   - cancel_accepted_seat       — 00200
--
-- Status-only check stays in place too: `cancelled` / `finished` /
-- `expired` activities remain locked through the existing path.
