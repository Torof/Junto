-- ============================================================================
-- 00365 — Post-code audit: invitations + join lifecycle.
--   • join_activity: add the `deleted_at` gate (its new `invited` branch could
--     otherwise convert an invitee into a member on an admin-removed activity).
--   • accept_activity_invitation: GET DIAGNOSTICS — if the invited row vanished
--     mid-call (concurrent block/decline), don't fire a phantom "accepted" push.
--   • send_activity_invitations: re-check the 24h cap inside the loop (the
--     single pre-loop check could overshoot by up to one batch).
-- ============================================================================
...
BEGIN
  ...
  -- deleted_at gate added (audit): a moderated (soft-deleted) activity keeps its
  -- status, so without this the invited branch could still make someone a member.
...
BEGIN
  ...
  UPDATE participations
  SET status = 'accepted', created_at = now(), left_at = NULL, refused_at = NULL
  WHERE id = v_row.id AND status = 'invited';
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  -- Row vanished between the SELECT and the UPDATE (concurrent decline / block
  -- cascade, which don't take the activity lock) → no phantom success/push.
