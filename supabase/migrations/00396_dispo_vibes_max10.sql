-- ============================================================================
-- 00396 — Raise the max selectable vibe pills 6 → 10 (Scott 2026-08-29). Pills
-- are core to partner matching, so a person can express a richer profile.
--   • discovery_availabilities_intent_check: cardinality 1..6 → 1..10
--   • upsert_dispo: cardinality guard > 6 → > 10 (reproduced from 00395)
-- (25-value vocabulary unchanged.)
-- ============================================================================
...
ALTER TABLE discovery_availabilities
  ...
  CHECK (
    ...
    OR (intent <@ ARRAY[
        ...
        AND coalesce(array_length(intent, 1), 0) BETWEEN 1 AND 10)
  );
...
BEGIN
  ...
  -- Vibe pills: optional, ≤10, closed vocabulary. Empty array normalises to NULL.
